 ...
 *
 * @param id Unique identifier for this source (e.g., "mattpocock", "internal")
 * @param url Git repository URL
 * @param ref Branch, tag, or commit hash to fetch
 * @param cacheDir Optional override for cache directory (defaults to .sbt-skills/sources/{id})
 */
 ...
 *
 * @param id Unique skill identifier (e.g., "code-review")
 * @param sourceId ID of the source this skill comes from
 * @param category Category path (e.g., "engineering", "productivity/advanced")
 * @param path Relative path to SKILL.md in the repository
 * @param version Git commit hash when fetched
 * @param harnessesInRepo Harnesses declared in SKILL.md frontmatter
 * @param effectiveHarnesses Harnesses after applying source-level overrides
 * @param lastFetched Timestamp when skill was last synced
 * @param customized Whether this skill has been customized (for Phase 2)
 */
 ...
 *
 * @param backend "file" or "git" (only "file" supported in Phase 1)
 * @param metricsFile Local file path for file-based backend
 * @param metricsGitRepo Git repository URL for git-based backend (Phase 3)
 * @param metricsGitBranch Branch in metrics repo (Phase 3)
 */
